...
from custom_components import apcupsd2
...
def setup_platform(hass, config, add_entities, discovery_info=None):
    ...
    for resource in config[CONF_RESOURCES]:
        ...
        if sensor_type.upper() not in apcupsd2.DATA.status:
            _LOGGER.warning(
                "Sensor type: %s does not appear in the APCUPSd status output",
                sensor_type,
            )

        entities.append(APCUPSdSensor(apcupsd2.DATA, sensor_type))
